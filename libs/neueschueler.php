...
function neueschuelerabfrage(){
  ...
  $sql    = "SELECT DISTINCT *
  FROM `export2`
  LEFT JOIN `export1` ON export1.`name` = benutzername
  WHERE `benutzerstatus` = '1'
  AND `User Logon Name` IS NULL
  AND `regelklasse` LIKE '%17%'";
  //
